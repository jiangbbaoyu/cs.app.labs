...
void transpose_submit(int M, int N, int A[N][M], int B[M][N])
{
    ...
    else if (N == 64)
    {
        for (int i = 0; i < N; i += 8)
        {
            for (int j = 0; j < M; j += 8)
            {
                // because only has 32 sets
                // when matrix is 64x64,  and divided to 8x8 blocks, each block row need 8 set.
                // 32 sets only accommodate 4 block rows, for the left 4 block rows will overwrite the first 4 block rows
                for (int k = i; k < i + 4; ++k)
                {
                    // 7/8  hit rates
                    int temp_value0 = A[k][j];
