...
0000000000401062 <phase_5>:
  ...
  // string at 0x4024b0 "maduiersnfotvbylSo you think you can stop the bomb with ctrl-c, do you?"
  // maduiersnfotvbyl can be treated as a hash table of size 16
  0  m
  1  a
  2  d
  3  u
...
00000000004010f4 <phase_6>:
  ...
  //  nested loop  below
  int i =0;     // use %r12d
  int last =0 ; // use %rbp, %r13
  while true {
    if num[last]>6 {  //  num[last] use %eax
      bomb !!
    }
...
000000000040145c <read_six_numbers>:
  ...
  40149d:	c3                   	retq
 // before the func sscanf called, the stack frame look likes
    high         |     ....        |
                 |_________________|  \
     (%rsp+8)->  |_______p6________|    \
       (%rsp)->  |_______p5________|4    |
       (%r9) ->  |_______p4________|4    |
       (%r8) ->  |_______p3________|4    |  phase_2 stack frame
       (%rcx)->  |_______p2________|4    |
       (%rdx)->  |_______p1________|4    |
                 | return address  |8  /
                 |_________________|/
                 |                 |\
                 |       ....      |  \
                 |_________________|     \
                 |   param6 addr   |8       |
                 |_________________|         |  read_six_numbers stack frame
                 |   param5 addr   |8        |
                 |_________________| <- %rsp |
                 |                 |
                 |                 |
                 |                 |
                 |                 |
